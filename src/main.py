...
def voice_change(voice_model, vocals_path, output_path, pitch_change, f0_method, index_rate, filter_radius, rms_mix_rate, protect, crepe_hop_length, fcpe_hop_length, fcpe_threshold, is_webui):
    ...
    # convert main vocals
    rvc_infer(rvc_index_path, index_rate, vocals_path, output_path, pitch_change, f0_method, cpt, version, net_g, filter_radius, tgt_sr, rms_mix_rate, protect, crepe_hop_length, vc, hubert_model, fcpe_hop_length, fcpe_threshold)
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
                        ...
                        reverb_dry=0.8, reverb_damping=0.7, output_format='mp3', progress=gr.Progress()):
    try:
        ...
        f0_suffix = ""
        if f0_method in ["mangio-crepe", "fcpe"]:
            hop_value = crepe_hop_length if f0_method == "mangio-crepe" else fcpe_hop_length
            f0_suffix = f"_{hop_value}"
        ai_vocals_path = os.path.join(song_dir, f'{os.path.splitext(os.path.basename(orig_song_path))[0]}_{voice_model}_p{pitch_change}_i{index_rate}_fr{filter_radius}_rms{rms_mix_rate}_pro{protect}_{f0_method}{f0_suffix}.wav')
        ai_cover_path = os.path.join(song_dir, f'{os.path.splitext(os.path.basename(orig_song_path))[0]} ({voice_model} Ver).{output_format}')

        if not os.path.exists(ai_vocals_path):
            display_progress('[~] Converting voice using RVC...', 0.5, is_webui, progress)
            voice_change(voice_model, main_vocals_dereverb_path, ai_vocals_path, pitch_change, f0_method, index_rate, filter_radius, rms_mix_rate, protect, crepe_hop_length, fcpe_hop_length, fcpe_threshold, is_webui)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-fchop', '--fcpe-hop-length', type=int, default=128, help='Hop length for FCPE pitch estimation. Lower values improve tracking with longer processing times.')
    parser.add_argument('-fct', '--fcpe-threshold', type=float, default=0.05, help='Periodicity threshold for FCPE (0-1). Higher values are more conservative and may ignore uncertain pitch frames.')
    ...
    cover_path = song_cover_pipeline(args.song_input, rvc_dirname, args.pitch_change, args.keep_files,
                                     ...
                                     crepe_hop_length=args.crepe_hop_length, fcpe_hop_length=args.fcpe_hop_length,
                                     fcpe_threshold=args.fcpe_threshold, protect=args.protect,
