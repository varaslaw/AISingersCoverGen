...
def select_device(preferred_device=None):
    """Select compute device and precision flags.

    Returns (device, is_half, message)
    """
    message = ""
    device = None
    if preferred_device:
        ...
        if normalized != "auto":
            ...
            if normalized.startswith("cuda") and not torch.cuda.is_available():
                message = "CUDA was requested but is not available. Falling back to CPU."
                device = "cpu"
            elif normalized == "cpu":
                device = "cpu"
            elif normalized == "mps":
                if torch.backends.mps.is_available():
                    ...
                    message = "MPS was requested but is not available. Falling back to CPU."
                    device = "cpu"
    ...
    if device is None:
        if torch.cuda.is_available():
            ...
            device = "cpu"
            message = "GPU not available. Using CPU may lead to slower performance."

    is_half = device.startswith("cuda")
    if device == "cpu":
        is_half = False

    return device, is_half, message
...
def voice_change(
    ...
    status_message = f"[~] Using device: {config.device} ({'FP16' if config.is_half else 'FP32'})"
    if device_message:
